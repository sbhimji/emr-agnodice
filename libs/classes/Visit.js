...
class Visit {
    ...
    async getById(id) {
        ...
        await pool
            ...
            .then((res) => {
                ...
                this.leuko = res.rows[0].leukocytes;
                this.nitrite = res.rows[0].nitrite;
                this.uro = res.rows[0].urobilinogen;
                this.protein = res.rows[0].protein;
                this.ph = res.rows[0].ph;
                this.blood = res.rows[0].blood;
                this.spec = res.rows[0].specific_gravity;
                this.ketone = res.rows[0].ketone;
                this.bili = res.rows[0].bilirubin;
                this.glucose = res.rows[0].glucose;
            })
            .catch((err) => {
    ...
    async addUrine(leuko, nitrite, uro, protein, ph, blood, spec, ketone, bili, glucose) {
        const text = `
        UPDATE public."visit"
        SET leukocytes = $1, nitrite = $2, urobilinogen = $3, protein = $4, ph = $5, blood = $6, specific_gravity = $7, ketone = $8, bilirubin = $9, glucose = $10
        WHERE visit_id = $11
        `
        const values = [leuko, nitrite, uro, protein, ph, blood, spec, ketone, bili, glucose, this.visit_id];
        await pool
            .query(text, values)
            .then((res) => {
                this.leuko = leuko;
