...
const Visit = require('../classes/Visit.js');
    ...
    await pool
      ...
      .then(async (res) => {
        await Promise.all(res.rows.map(async (row) => {
          await this.medicalHist.push([row.condition_, row.yr_occured, row.medications, row.dose_and_freq, row.hosp]);
        }));
      })
      .catch((err) => console.error('Error executing query', err.stack));
  }

  async addPatient(fName, lName, sex, birthday, age, insurance, pcp) {
    const text = `
    INSERT INTO public."patientGenInfo"(
    fname, lname, sex, birthday, age, insurance, pcp)
    VALUES ($1, $2, $3, $4, $5, $6, $7);
    `
    const values = [fName, lName, sex, birthday, age, insurance, pcp];
    await pool
    .query(text, values)
    .then((res) => {
      this.fName = fName;
...
async getPatientId() {
  ...
  async addSocial(occupation, diet, exercise, curr_hous, yr3_hous, caffeine, alc, packs_day, yrs, pack_yrs, quit, marijuana, fun_drugs, pets,
    ...
    SET soc_hist_occ = $1, soc_hist_diet = $2, soc_hist_exc = $3, curr_housing = $4, yr3_housing = $5,
        ...
        fun_drugs = $13, sex_active = $14, last_year_part = $15, protection_use = $16, sti = $17, pets = $18
    WHERE patient_id = $19
    `
    const values = [occupation, diet, exercise, curr_hous, yr3_hous, caffeine, alc, packs_day, yrs, pack_yrs, quit, marijuana, fun_drugs,
    sex_act, partners, protection, sti, pets, this.patient_id];
...
async addFamily(mother, father, siblings, children, sig_other, visit_id) {
  ...
  async addMedHist(condition, yr_occ, medications, dose, hosp) {
    const text = `
        INSERT INTO public."med-hist"
        VALUES ($1, $2, $3, $4, $5, $6)
        `;
    const values = [this.patient_id, condition, yr_occ, medications, dose, hosp];
    await pool
        .query(text, values)
        .then((res) => {})
        .catch((err) => console.log('Error executing query', err.stack))
